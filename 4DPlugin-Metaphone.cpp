...
static int IsVowel(const string &s, size_t pos)
{
  char c;
...
static bool SlavoGermanic(const string &s)
{
  return s.find('W') != string::npos ||
         s.find('K') != string::npos ||
         s.find("CZ") != string::npos ||
         s.find("WITZ") != string::npos;
}

static char GetAt(const string &s, size_t pos)
{
  if (pos >= s.length()) {
    return '\0';
  }
...
static void SetAt(string &s, size_t pos, char c)
{
  if (pos >= s.length()) {
    return;
  }
...
static int StringAt(const string &s, unsigned int start, unsigned int length, ...)
{
  const char *test;
  const char *pos;
  va_list ap;
...
static void push_back(PA_CollectionRef metaphones, const std::string& str)
{
    if (!metaphones)
        return;
...
static void DoubleMetaphoneImplementation(string &str, PA_CollectionRef codes)
{
  ...
  MakeUpper(original);
  const bool slavoGermanic = SlavoGermanic(original);
  ...
  if (primary.length() > max_length)
    primary.resize(max_length);

  if (secondary.length() > max_length)
    secondary.resize(max_length);

  push_back(codes, primary);
  push_back(codes, secondary);
}

static void u16_to_macroman(const PA_Unistring *word, std::string& word_macroman)
{
    word_macroman.clear();
    if (!word || !word->fString || word->fLength == 0)
        return;

    const uint32_t len = static_cast<uint32_t>(word->fLength * sizeof(PA_Unichar)) + 1;
    ...
    // Guarantee NUL termination before constructing the std::string.
    buf[len - 1] = 0;
    word_macroman.assign(reinterpret_cast<const char *>(buf.data()));
}
